...
try:
    ...
    def write_wav(*args, **kwargs):
        raise ImportError("scipy not found. Please install it (e.g., pip install scipy) to enable temporary file saving.")
...
class AudioDurationNode:
    ...
    def _extract_samples_sr(self, audio_obj):
        ...
        if isinstance(audio_obj, (list, tuple)):
            ...
            if len(audio_obj) == 2 and not isinstance(audio_obj[0], (list, tuple, dict)):
    ...
    def _probe_seconds_ffprobe(self, path: str) -> float:
        ...
        try:
            ...
            raise RuntimeError(f"ffprobe returned a non-numeric duration for {path!r}: {out!r}")
        if secs < 0:
    ...
    def _duration_from_samples(self, audio_obj: Any) -> Optional[float]:
        ...
        try:
            if _torch is not None and isinstance(samples, _torch.Tensor):
                samples = samples.detach().cpu().numpy() # Convert to numpy for consistent handling
                _np = np
            ...
            if _np is not None and isinstance(samples, _np.ndarray):
                ...
                if hasattr(samples, "__getitem__") and len(samples) > 0 and hasattr(samples[0], "__len__"):
    ...
    def _save_audio_to_temp(self, samples: Any, sr: float) -> Optional[str]:
        ...
        try:
            ...
            if hasattr(samples, 'detach') and hasattr(samples, 'cpu') and hasattr(samples, 'numpy'):
            ...
            if samples.ndim == 2 and samples.shape[0] < samples.shape[1]:
                ...
                samples = samples.reshape(-1, samples.shape[-1]) if samples.shape[-1] < 10 else samples.flatten()

            # Normalize data type and range for WAV saving (e.g., int16)
            # Find the max absolute value for normalization
            max_val = np.abs(samples).max()
            if max_val > 1.0:
                # If values exceed the float range [-1.0, 1.0], assume they are already integers
                dtype = samples.dtype
            else:
            ...
            output_dir = os.path.join(tempfile.gettempdir(), 'comfyui_temp', 'audio_duration_cache')
            os.makedirs(output_dir, exist_ok=True)
            
            temp_path = os.path.join(output_dir, f"temp_{os.getpid()}_{os.urandom(4).hex()}.wav")
    ...
    def run(self, audio_path: str, audio: Any = None) -> Tuple[int, float, int, float]:
        ...
        try:
            ...
            if not path_to_probe and audio is not None:
                ...
                # b) If no path, try duration or samples/sr
                if path_to_probe is None:
                    # Check for precomputed duration OR calculate from samples/sr
                    seconds = self._duration_from_samples(audio)
    
                    # c) If duration still not found, save samples/sr to a temp file
                    if seconds is None:
                        samples, sr = self._extract_samples_sr(audio)
                        if samples is not None and sr is not None:
                            # Save the audio data to a temp file and get the path
                            path_to_probe = self._save_audio_to_temp(samples, sr)
                            temp_file_to_cleanup = path_to_probe # Mark for cleanup

            # If we still don't have seconds, probe by path
            if seconds is None:
                if not path_to_probe:
                    raise RuntimeError(
                        "Provide either a valid audio_path or connect an AUDIO input that contains a usable path/samples."
                    )
            ...
            if seconds is None or seconds <= 0:
                 raise RuntimeError(f"Could not determine a valid duration for the audio source: {path_to_probe or 'AUDIO input'}")
            ...
            # Cleanup the temporary file if one was created
            if temp_file_to_cleanup and os.path.exists(temp_file_to_cleanup):
                try:
                    os.remove(temp_file_to_cleanup)
                except Exception as e:
                    print(f"Warning: Failed to cleanup temporary audio file {temp_file_to_cleanup}: {e}")
